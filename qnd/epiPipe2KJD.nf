...
def parseParams(){
    ...
    if (!(params.biosample || params.fastqDir || params.genewizMap) ){exit 1, "You need to use either --biosample aBaseSpace-biosample or --fastqDir path/to/bam/files or --genewizMap mappingFiles.csv. Use --help to get the full usage." }
    if ( !(params.outDir) ){ exit 1, "--outDir is a required arguments. Use --help to get the full usage." }
    
    if (params.resolutions){
        params.resolutionsList = params.resolutions.split(/,/,-1)
    } else {
        params.resolutionsList = [5,10]
    }

    if (params.ABresolutions){
        params.ABresolutionsList = params.ABresolutions.toString().split(/,/,-1)
    } else {
        params.ABresolutionsList = [32,64,128]
    }
}
